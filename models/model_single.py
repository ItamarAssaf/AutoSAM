...
class ModelEmb3D(nn.Module):
    ...
    def forward(self, img):
        ...
        for idx, layer in enumerate(self.backbone):
            ...
            print(f"➡ Passing through backbone layer {idx} with input shape {x.shape} (dtype: {x.dtype})")
            x = layer(x)
            print(f"✅ Layer {idx} done in {time.time() - start_layer:.2f} seconds. Output shape: {x.shape}")
            z.append(x)

        print(f"🔥 Backbone forward done in {time.time() - start_all:.2f} seconds. Starting decoder.")

        start_decoder = time.time()
        # FIX: pass correct order of feature maps to decoder
        dense_embeddings = self.decoder(z)
        print(f"✅ Decoder finished in {time.time() - start_decoder:.2f} seconds. Output shape: {dense_embeddings.shape}")

        dense_embeddings = F.interpolate(
            dense_embeddings, size=(32, 256, 256), mode='trilinear', align_corners=True
        )

        print(f"⭐ Interpolation done. Final dense embedding shape: {dense_embeddings.shape}")
        return dense_embeddings
